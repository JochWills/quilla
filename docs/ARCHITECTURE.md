...
- `records` — one row per Record of Advice.
  ...
  - `data` (jsonb): the full record state from `blank()` in `app/js/app.js`:
    ...
    - `audit` [{at, text}]
- `profiles` — one row per advisor (`id` = `auth.users.id`, auto-created on sign-up by a trigger): `full_name`, `fsp_number`, `practice_name`, `template` (jsonb `{section_id: {guidance, standard}}`, max 60 KB). Edited in the Account settings popup and on the Templates screen; prefills `meta.adviser`/`meta.fsp` on new records. The `ai` function reads `template` server-side (guidance only shapes wording, never facts); the app appends `standard` wording to sections after drafting and marks them.
...
- `records.archived_at` — set when the advisor archives a record (hidden from the main list, shown under the Archived tab; nothing else changes). Archiving is the suggested alternative to deleting signed records, which FAIS requires keeping for five years.
